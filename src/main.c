/*PAR MAIBECHE MASSIOUANE*/

#define _GNU_SOURCE
#define _POSIX_C_SOURCE 1999309L
#define AJUSTEMENT_EXTRATIME 3
/*Uniquement car on utilise clock_gettime, car ce n'est pas ansi C89'*/
...
static void aff_plat(Plateau *plat){
    ...
    for(i = 0; i < LARGEUR_PLATEAU; ++i){
        for(j = 0; j < LONGEUR_PLATEAU; ++j)
            plat->plateau[i][j] ? printf("%d ", plat->plateau[i][j]) : printf("  ");
        printf("\n");
    }
    printf("\n\n");
}
...
int main(int argc, const char * const argv[]){
    ...
    srand(time(NULL));
    MLV_create_window("Stealth", "stealth", LONGUEUR_FENETRE + EXTENSION, LARGEUR_FENETRE);
    create_map(&plateau);
    affiche_menu();
    ...
    while(fin_de_partie == 0){
        ...
        /*Mecanisme de jeu et gestion de terrain*/
